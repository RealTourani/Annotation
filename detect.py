...
obj_file = "config/coco.names"
obj_classes = []
net_config = "config/yolov3-tiny-prn.cfg"
net_weights = "config/yolov3-tiny-prn_final.weights"
blob_size = 320
...
def findObjects(output, img):
    ...
    for i in indices:
        ...
        cv2.rectangle(img, (x+20,y+130), (x+w, y+h), (0,255,0), 2)
        cv2.putText(img, f'{obj_classes[class_ids[i]].upper()} {int(confidences[i] * 100)}%',
                    (x, y+70), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
